// Stock market data service for investment advice
// This is a mock service - in production, you would integrate with real stock APIs
...
const SECTOR_ETFS = {
  ...
  "Financial Services": ["XLF", "VFH", "FXO"],
...
export const getSectorRecommendations = (sectors) => {
  const recommendations = [];

  sectors.forEach((sector) => {
    if (SECTOR_ETFS[sector]) {
      recommendations.push({
        sector,
        etfs: SECTOR_ETFS[sector].map((etf) => ({
          symbol: etf,
          name: getETFName(etf),
        })),
      });
